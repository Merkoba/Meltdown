...
def init_osx_pbcopy_clipboard():
    def copy_osx_pbcopy(text):
        ...
        p.communicate(input=text.encode(ENCODING), timeout=TIMEOUT)
...
def init_xclip_clipboard():
    DEFAULT_SELECTION = "clip"
    PRIMARY_SELECTION = "clip"
    ...
    def copy_xclip(text, primary=False):
        ...
        p = subprocess.Popen(
            ["xclip", "-sel", selection], stdin=subprocess.PIPE, close_fds=True
        )
        p.communicate(input=text.encode(ENCODING), timeout=TIMEOUT)
